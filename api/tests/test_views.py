...
class TestGenericsViews(TestCase):
    ...
    def test_review_perform_create(self):
        response4 = self.client.post('http://127.0.0.1:8000/api/reviews/',{'id':"10", 'title':"test_review", 'rating':"5",'ip_address':"123.123.123.123",'summary':"testing",
                                                                          'submission_date':"2018-07-07",'company':self.company1})
...
class TestAuthViews(TestCase):
    ...
    def test_login(self):
        response = self.client.post('http://127.0.0.1:8000/api/login/',{'username': self.user1.username, 'password':self.user1})
        self.assertEqual(response.status_code, status.HTTP_302)
